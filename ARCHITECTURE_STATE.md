...
| **MathAnswerEvaluator** | `js/core/math-answer-evaluator.js` | Chuẩn hóa đáp án `cleanAnswer`, so khớp đáp án điền số `evaluateShortAnswer` (dung sai 40%, bóc tách đơn vị). | **ĐÃ HOÀN THÀNH (ACTIVE)** | 100% unit tests (`tests/core/math-answer-evaluator.test.js`) |
...
| **MathQuestionClassifier** | `js/core/math-question-classifier.js` | Phân loại câu hỏi trắc nghiệm bắt buộc vs điền số ngắn (`shouldForceMCQ`). | **ĐÃ HOÀN THÀNH (ACTIVE)** | 100% unit tests (65 tests trong `tests/core/should-force-mcq.characterization.test.js`) |
| **MathTemplateCompiler** | `js/core/math-template-compiler.js` | Biên dịch template câu hỏi AI (`generateQuestionFromTemplate`), xáo trộn phương án và đồng bộ `{ans_letter}`. | **CHARACTERIZED / SẴN SÀNG ĐỀ XUẤT** | 100% characterization tests (15 tests trong `tests/core/math-template-compiler.characterization.test.js`) |
