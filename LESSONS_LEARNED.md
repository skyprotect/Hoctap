...
* **Quy chuẩn**:
  ...
  - Máy chủ sử dụng `google-auth-library` để verify ID token độc lập, trích xuất UID từ `payload.sub` (Google Subject ID), cấp phát JWT phiên làm việc và lưu session vào SQLite cục bộ. Không bắt buộc phải có `firebaseUid` từ client.
...
### 2. Không Chặn Học Sinh Bằng Màn Hình Đăng Nhập (Instant Workspace Entry)
* **Bài học**: Ứng dụng học tập Kiosk tuyệt đối không được chặn học sinh ở màn hình đăng nhập Google hay hiện pop-up lỗi mạng khiến học sinh bế tắc.
* **Quy chuẩn**:
  - Khi khởi động, ứng dụng luôn tự động vào ngay không gian học tập với hồ sơ mặc định (**Trần Bình Minh** - Lớp 6).
  - Quá trình kiểm tra phiên Google và đồng bộ đám mây diễn ra ngầm (silent background). Phụ huynh có thể chủ động liên kết Google bất kỳ lúc nào từ màn hình Quản trị.

### 3. Nguyên Tắc Khởi Tạo Giao Diện Ngay Lập Tức (Instant UI Initialization)
...
### 4. Tự Trị Đóng Gói Hoàn Toàn (Zero-Config Seeding)
...
### 5. Đóng Gói Tài Nguyên Ngoại Tuyến & Cơ Chế Dự Phòng (Offline First & CDN Fallback)
...
* **Quy chuẩn**:
  ...
  - Phục vụ đúng Content-Type (`font/woff2`, `font/ttf`) trong Express static headers.

### 6. Quản Lý Trạng Thái & Bộ Nhớ Sạch Sẽ (State Isolation & Clean Memory)
