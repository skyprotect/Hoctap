...
const MathExprEvaluator = (typeof window !== 'undefined' && window.MathExprEvaluator) 
...
const questions = {
    ...
    initPractice: function(lesson, level = 'co-ban', isExam = false) {
        ...
        if (this.isLessonExamMode) {
            ...
            const fallbackToLocalGenerators = (reason) => {
                ...
                this.currentQuestionIndex = 0;
                Swal.close();
                // [BUG FIX v13.56] Phải hiển thị câu hỏi trực tiếp thay vì quay về màn hình chọn chế độ
                    ...
                    if (questions && questions.length > 0) {
                        ...
                        worker.onmessage = (e) => {
                            ...
                            if (response.status === 'success') {
                                ...
                                this.currentQuestionIndex = 0;
                                // [BUG FIX v13.56] Phải chuyển thẳng vào làm bài thay vì quay về màn hình chọn chế độ
                                this.practiceMode = 'standard';
                                document.getElementById("practice-mode-select-box").classList.add("hidden");
                                document.getElementById("practice-active-box").classList.remove("hidden");
                                this.showQuestion();
    ...
    loadChatLuongCaoExam: function(lesson, types) {
        // [BUG FIX v13.56] Chốt chặn an toàn: nếu lesson là null thì fallback cục bộ ngay lập tức
        if (!lesson || !lesson.id) {
            console.warn('[loadChatLuongCaoExam] lesson hoặc lesson.id bị null/undefined, kích hoạt bộ sinh đề dự phòng cục bộ.');
            const safeTypes = types && types.length > 0 ? types : ['tap-hop'];
            const fallbackQuestions = [];
            const diffDist = ['kho', 'kho', 'kho', 'kho', 'kho', 'kho', 'kho', 'kho', 'nang-cao', 'nang-cao', 'nang-cao', 'nang-cao', 'kho', 'kho', 'kho', 'kho'];
            for (let i = 0; i < 16; i++) {
                const rType = safeTypes[Math.floor(Math.random() * safeTypes.length)];
                const q = this.generateQuestion(rType, diffDist[i]);
                q.isSpacedRepetition = false;
                q.level = 'chat-luong-cao';
                q.type = rType;
                fallbackQuestions.push(q);
        ...
        fetch(this.getApiUrl(`/api/get-questions?lessonId=${lesson.id}&lessonTitle=${encodeURIComponent(lesson.title || '')}&classLevel=${(window.app && app.config && app.config.currentClass) || '6'}&studentId=${studentId}`))
            .then(res => {
                if (!res.ok) throw new Error('Không thể kết nối với server.');
                return res.json();
            })
    ...
    aiTroubleshoot: async function() {
        const q = this.currentQuestions && this.currentQuestions[this.currentQuestionIndex];
        // [BUG FIX v13.56] Nếu không có câu hỏi (lỗi tải đề), thông báo và gợi ý thoát
        if (!q) {
            Swal.fire({
                icon: 'warning',
                title: 'Chưa có câu hỏi nào',
                html: 'Đề thi AI chưa tải được câu hỏi. Hãy nhấn nút <b>← Thoát</b> để quay lại và thử lại sau.',
                confirmButtonText: 'Đã hiểu',
                confirmButtonColor: 'var(--primary)',
                target: document.getElementById('tab-practice') || 'body'
            });
